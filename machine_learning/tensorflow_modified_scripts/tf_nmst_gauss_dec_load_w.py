...
class CVAE(tf.keras.Model):
  ...
  @tf.function
  def sample(self, eps=None, use_stoich=True):
    if eps is None:
      eps = tf.random.normal(shape=(100, self.latent_dim))
    return self.decode(eps, apply_sampling=use_stoich)
...
#       z = tf.reshape(z, [-1])
      x_decoded = model.sample(z, use_stoich=False)
      '''
      for k in range(1):
        mean, logvar = model.encode(x_decoded)
